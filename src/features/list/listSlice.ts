  ...
  reducers: {
    ...
    deleteList: (
      ...
      if (!(listId in state.allLists)) {
        throw new Error(
          'The list you want to delete does not exist',
        );
      } else {
    ...
    updateCurrentList: (
      ...
      if (!(action.payload.listId in state.allLists)) {
        throw new Error(
          'The project you want to update does not exist',
        );
      } else {
        state.allLists[action.payload.listId] =
          action.payload;
      }
    },
    // ******************************************************************************************************
    ...
    updateListsOrder: (
      ...
      if (!(draggableId in state.allLists)) {
        throw new Error(
          'The list you want to move does not exist',
        );
      }
      if (!destination) {
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        console.log(
          'Unexpected case. Need to figure out what I missed here',
        );
      }
    },
    updateColumnsOrderInList: (
      ...
      if (
        !state.allLists[
          sourceListId
        ].columnIdsOrder.includes(draggableId)
      ) {
        throw new Error(
      ...
      const [remove] = state.allLists[
        sourceListId
      ].columnIdsOrder.splice(source.index, 1);
