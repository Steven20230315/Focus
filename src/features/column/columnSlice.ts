...
const columnSlice = createSlice({
  ...
  reducers: {
    ...
    updateTasksOrderInColumn(
      ...
      const sourceColumn =
        state.allColumns[source.droppableId];

      const destinationColumn =
        state.allColumns[destination!.droppableId];
      if (!(sourceColumn.columnId in state.allColumns)) {
        throw new Error(
          'The source column does not exit in allColumns',
        );
      }
      if (
        !(destinationColumn.columnId in state.allColumns)
      ) {
        throw new Error(
          'The destination column does not exit in allColumns',
        );
      }
      if (
        !state.allColumns[
      ...
      const oldIndex = source.index;
      const newIndex = destination.index;
      const [remove] = state.allColumns[
        source.droppableId
      ].taskIds.splice(oldIndex, 1);
      state.allColumns[
        destination.droppableId
      ].taskIds.splice(newIndex, 0, remove);
    },
    updateTaskOwner(
      state: ColumnState,
      ...
      const newColumn =
        state.allColumns[destination!.droppableId];
      const [remove] = currentColumn.taskIds.splice(
        source.index,
        1,
      );
  ...
  extraReducers: (
    builder: ActionReducerMapBuilder<ColumnState>,
  ) => {
    builder
      .addCase(
        addListWithDefaultColumns,
        (
          ...
          const { columnIdsOrder: columnIds, listId } =
            action.payload;
          roles.forEach(
            (role: ColumnRole, index: number) => {
      ...
      .addCase(
        ...
        (
          ...
          const { columnIdsOrder: columnIds } =
            action.payload;
      ...
      .addCase(
        ...
        (
          ...
          const { columnId, taskId } = action.payload;
          const deleteTaskIdFromThisColumn =
            state.allColumns[columnId];
          deleteTaskIdFromThisColumn.taskIds.splice(
            deleteTaskIdFromThisColumn.taskIds.indexOf(
              taskId,
            ),
            1,
          );
